...
class VideoLibrary:
    ...
    def get_all_videos(self):
        ...
        " Sorts the list of videos by video titles alphabetically"
        video_list.sort(key= lambda x: x.title)
        return video_list

    def get_unflagged_videos(self):
        """Returns all available unflagged video information from the video library."""
        video_list = list(self._videos.values())
        return_list = []
        "Removes flagged videos from the list"
        for video in video_list:
            if not video.flagged:
                return_list.append(video)
        " Sorts the list of videos by video titles alphabetically"
        return_list.sort(key= lambda x: x.title)
        return return_list
